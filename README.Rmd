...
functions `getPacfinSppData` and `getPacfinSppData`. 

The `getPacfinSppData(year, save = F, fromFile = F)` function prepares an R 
data.frame from the neccessary PacFIN calls needed to perform a species expansion.
By default this function connects to the PacFIN database to prepare its 
output. If run with `save=True`, then the output will be saved to a local csv file 
for replicability and/or convience purposes. If a local csv is present on your 
system, `getPacfinSppData` can be run with `fromFile=True` to bypass VPN and 
...
they imply see the R help pages for these data structes (i.e. `?portMatrix2' or 
...
Stripping out the commentary from the above commands results in the following simple example of an expansion in 2019.
```
